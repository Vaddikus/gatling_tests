...
class TestLoginApi extends Simulation {

  object Login {

    val httpConf = http
      .baseURL(settings.Settings.getURL("loginApi"))
    ...
    val loginByEmail = exec(
      ...
      .exec(
        //It is a fake GET-request. It's need to get non-empty body and substitute it
        //This test checks if players with the same names can login only to own site
